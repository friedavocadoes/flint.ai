      ...
      <div className="z-[100] fixed w-full top-0 h-14 bg-background/40 flex items-center px-4 md:px-10 backdrop-blur-md font-outfit">
        <div className="flex items-center min-w-0">
          <Link href="/" className="group flex items-center gap-1.5">
            <p className="font-bold text-2xl bg-gradient-to-tr dark:from-white/70 dark:to-white/20 dark:hover:text-stone-300 from-black/70 to-black/20 hover:text-stone-800 bg-clip-text text-transparent drop-shadow-[0_2px_16px_rgba(255,255,255,0.25)] transition-all duration-400 ease-in-out">Flint.ai</p>
          </Link>
          {paths.map((path) => pathname === path.p && <div className="flex items-center" key={path.p}><span className="ml-2 text-xl font-semibold text-muted-foreground/60">/</span><span className="ml-1 text-xl font-semibold text-muted-foreground">{path.name.replace("/", "")}</span></div>)}
        </div>
        ...
        <div className="hidden ml-auto md:flex items-center">
          {!user ? <><Button className="mx-2" onClick={() => router.push(routes.auth.signupRoute)}><UserPlus />Sign Up</Button><Button className="mx-2" variant="secondary" onClick={() => router.push(routes.auth.loginRoute)}><LogIn />Log in</Button></> : <div className="mx-2"><UserDropDown name={user.name} email={user.email} pro={user?.pro} onLogout={clearUser} /></div>}
          <ModeToggle />
        </div>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56">
        ...
        <DropdownMenuGroup>
          ...
          {pro ? <DropdownMenuItem onClick={() => router.push(routes.sub)}><CreditCard /><span>Billing</span></DropdownMenuItem> : <DropdownMenuItem onClick={() => router.push(routes.sub)}><CircleFadingArrowUp /><span>Upgrade to Pro</span></DropdownMenuItem>}
        </DropdownMenuGroup>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => window.open("https://github.com/friedavocadoes/flint.ai", "_blank", "noopener,noreferrer")}><Github /><span>GitHub</span></DropdownMenuItem>
        <DropdownMenuItem onClick={() => window.open(routes.static.contact, "_blank", "noopener,noreferrer")}><LifeBuoy /><span>Support</span></DropdownMenuItem>
        <DropdownMenuItem disabled><Cloud /><span>API <span className="ml-auto text-[10px] text-muted-foreground">Soon</span></span></DropdownMenuItem>
        <DropdownMenuSeparator />
