...
const tools = [
  { name: "ResumeAI", href: staticRoutes.resume, icon: FileText, description: "Analyze & improve your resume" },
  { name: "PrepareAI", href: staticRoutes.prepare, icon: Compass, description: "Build your career roadmap" },
  { name: "Discussions", href: staticRoutes.discussions, icon: MessageCircle, description: "Explore ideas with the community" },
];

const support = [
  { name: "Documentation", href: staticRoutes.static.documentation, icon: BookOpen },
  { name: "Contact Us", href: staticRoutes.static.contact, icon: MessageCircle },
    ...
    >
        ...
        {user ? (
          ...
          >
            <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-to-br from-violet-500/30 to-cyan-400/20 text-sm font-bold ring-1 ring-white/10">
              {user.avatar ? <img src={user.avatar} alt="" className="h-10 w-10 rounded-full object-cover" /> : user.name?.charAt(0).toUpperCase()}
            </div>
          ...
          <div className="mt-4 rounded-2xl border border-violet-400/20 bg-gradient-to-br from-violet-500/15 via-background to-cyan-400/10 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Sign in to unlock Flint's AI tools.</p>
            <div className="mt-3 flex gap-2">
              <Link href={routes.loginRoute} onClick={() => setOpenMobile(false)} className="flex-1">
                <Button variant="secondary" className="w-full">Log in</Button>
              </Link>
              <Link href={routes.signupRoute} onClick={() => setOpenMobile(false)} className="flex-1">
              ...
              {tools.map((tool) => {
                ...
                return (
                  <SidebarMenuItem key={tool.name}>
                    ...
                    >
                      <Link href={tool.href} onClick={() => setOpenMobile(false)}>
                        <Icon className="h-4 w-4" />
                        <span className="flex min-w-0 flex-col items-start gap-0.5">
                          <span className="font-medium">{tool.name}</span>
                          <span className="text-[10px] font-normal text-muted-foreground">{tool.description}</span>
                        </span>
                      </Link>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
        ...
        <SidebarGroup className="p-0">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-1">
              ...
              <SidebarMenuItem>
                <SidebarMenuButton asChild className="h-10 rounded-xl px-3">
                  <Link href={staticRoutes.sub} onClick={() => setOpenMobile(false)}>
                    <Crown className="h-4 w-4 text-amber-400" />
                    <span>Go Premium</span>
                    <span className="ml-auto text-[9px] font-semibold uppercase tracking-wider text-amber-400">Pro</span>
                  </Link>
                </SidebarMenuButton>
              </SidebarMenuItem>
              <SidebarMenuItem>
      ...
      <SidebarFooter className="border-t border-white/10 p-3">
        {user ? (
          ...
          <p className="px-2 py-2 text-center text-[10px] text-muted-foreground">AI tools for your next move.</p>
        )}
      </SidebarFooter>
    </Sidebar>
  );
