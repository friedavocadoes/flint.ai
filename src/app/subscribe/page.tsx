...
export default function SubscribePage() {
  ...
  function handlePaymentSuccess(product: "premium" | ChatProduct) {
    if (product === "premium") setSuccessProduct("premium");
    else setSuccessProduct(product);
  }
        ...
        {premiumActive ? (
          <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="relative mb-10 overflow-hidden rounded-[32px] border border-violet-500/25 bg-gradient-to-br from-violet-500/15 via-background to-cyan-500/10 p-8 md:p-12">
            ...
            <div className="relative flex flex-col gap-7 md:flex-row md:items-center md:justify-between">
              <div><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-500/25 bg-violet-500/10 px-3 py-1.5 text-xs font-semibold text-violet-600 dark:text-violet-300"><Crown className="h-3.5 w-3.5" /> Premium member</div><h1 className="text-3xl font-black tracking-tight md:text-5xl">You&apos;re unlocked.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Congratulations. You have unlimited access to Flint for the rest of your Premium term. Build different pathways, test alternatives and keep refining.</p>{endDate && <p className="mt-3 text-xs font-medium text-muted-foreground">Active until <span className="text-foreground">{endDate}</span></p>}</div>
              <div className="flex h-28 w-28 shrink-0 flex-col items-center justify-center rounded-3xl border border-violet-500/20 bg-background/60 shadow-lg"><Crown className="mb-1 h-6 w-6 text-violet-500" /><span className="text-3xl font-black">∞</span><span className="text-[10px] uppercase tracking-widest text-muted-foreground">access</span></div>
            </div>
          </motion.section>
        ) : (
