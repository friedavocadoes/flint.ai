...
export default function SubscribePage() {
  ...
  async function cancelPremium() {
    const normalized = confirmation.trim().replace(/\s+/g, " ").toUpperCase();
    if (!userInfo?.id) {
      toast.error("We couldn't identify your account. Please refresh and try again.");
      return;
    }
    if (normalized !== CANCEL_CONFIRMATION) {
      toast.error(`Type ${CANCEL_CONFIRMATION} exactly to confirm.`);
      return;
    }
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/api/cashfree/cancel-premium`, {
        ...
        body: JSON.stringify({ userId: userInfo.id, confirmation: normalized }),
      });
