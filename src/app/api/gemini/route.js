...
function normalizePrepareResponse(data) {
  ...
  if (!chat.summary) {
    chat.summary = (chat.overview || "").split(".")[0]?.slice(0, 120) || chat.title || "";
  }
  if (!chat.meta) {
    chat.meta = {
      chances: 50,
      ...
      timeline: "6-9 months",
      level: "Intermediate",
      commitmentFit: "Fits schedule",
    };
  }
  if (!chat.motivation) {
    chat.motivation = {
      streakTip: "25 min daily wins",
      nextWin: "Complete first quest to unlock next",
    };
  }
  if (!chat.progress) {
    chat.progress = {
  ...
  if (Array.isArray(stages)) {
    chat.flowjson.pathwayData.stages = stages.map((s, i) => ({
      ...
      subtitle:
        s.subtitle ??
        s.estimatedDuration ??
        `${s.estimatedHours ?? 12}h • ${s.difficulty ?? "Intermediate"}`,
      description: s.description ?? s.whyItMatters ?? "",
      icon:
        s.icon ??
        ["Rocket", "Code2", "Library", "Target", "Brain", "Users"][i % 6],
      type: s.type ?? "skill",
      difficulty: s.difficulty ?? "Intermediate",
      estimatedDuration: s.estimatedDuration ?? `${s.estimatedHours ?? 12}h`,
      estimatedHours: Number.isFinite(s.estimatedHours)
        ? s.estimatedHours
        : 12 + i * 4,
  ...
  if (!chat.flowjson?.structData?.nodes && stages) {
    chat.flowjson.structData = {
      ...
      edges:
        chat.flowjson.pathwayData.connections?.map((c) => ({
          source: c.from,
          target: c.to,
          animated: true,
          id: `reactflow__edge-${c.from}-${c.to}`,
        })) || [],
    };
  }
...
async function generateWithGemini(prompt) {
  ...
  try {
    ...
    return JSON.parse(jsonrepair(response.text));
  }
}
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      return Response.json(normalizePrepareResponse(data));
    } catch (geminiError) {
      console.error("Gemini PrepareAI error:", geminiError);

      if (!shouldFallbackFromGemini(geminiError)) {
        throw geminiError;
      }

      console.warn("Gemini unavailable; using Groq emergency fallback for PrepareAI");
      const fallbackData = await generateGroqJson(prompt);
      return Response.json(normalizePrepareResponse(fallbackData));
    }
  } catch (e) {
    console.error("PrepareAI provider error:", e);
